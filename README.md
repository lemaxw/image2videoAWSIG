...
- `IMAGE2JSON_MAX_IMAGE_SIDE`: longest image side sent to the vision model (default `1600`; use `1024` for unusually slow images without changing the full-resolution render input)
- `IMAGE2JSON_RETRIES`: retry count after a vision request timeout (default `1`; use `0` for bounded one-off retries)
- `IMAGE2JSON_SHORT_VERSION`: use image2json's shorter vision prompt/schema (default `false`; useful for a manual rerun when the detailed schema repeatedly times out)
